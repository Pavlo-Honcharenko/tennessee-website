...
Parallax.Each = class {
  constructor(parent) {
    ...
    this.offset = 0;
    this.value = 0;
    this.smooth = parent.dataset.flsParallaxSmooth ? Number(parent.dataset.flsParallaxSmooth) : 1;
    this.setEvents();
  }
  setEvents() {
  ...
  animationFrame() {
    ...
    if (positionParent.top < 3 && positionParent.bottom > -3) {
      switch (centerPoint) {
        ...
        case "center":
          this.offset = heightWindow / 15 - (topToWindow + heightParent / 15);
          break;
    ...
    this.elements.forEach((el) => {
      const parameters = {
        ...
        coefficient: el.dataset.flsParallaxCoefficient ? Number(el.dataset.flsParallaxCoefficient) : 1,
